    ...
    setup(){
      ...
      async function loadSongs(){
        ...
        try{
          ...
          const all = await Promise.all(jsons.map(file => fetch(RAW_BASE + file.name, { cache:'no-store' }).then(r => r.ok ? r.json() : null).catch(() => null)));
          songs.value = all.filter(Boolean).map(enrichSong).sort((a,b) => String(a.title || '').localeCompare(String(b.title || ''),'zh-Hans-CN'));
          const id = decodeURIComponent(location.hash.replace(/^#/,''));
          const match = songs.value.find(song => song.id === id);
          if(match) openSong(match, { fromUrl:true });
        }catch(err){
