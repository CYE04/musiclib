...
(function(){
  ...
  async function getSongJsonEntries(){
    try{
      const res=await fetch(GITHUB_API+'?t='+Date.now(),{cache:'no-store'});
      if(res.ok){
        const files=await res.json();
        return files
          .filter(f=>f&&f.name&&f.name.endsWith('.json')&&f.name!=='test.json')
          .map(f=>({name:f.name,url:RAW_BASE+encodeURIComponent(f.name)}));
      }
      console.warn('[musiclib] GitHub API unavailable:',res.status,'using jsDelivr fallback');
    }catch(err){
      console.warn('[musiclib] GitHub API request failed, using jsDelivr fallback',err);
    }

    const metaUrl='https://data.jsdelivr.com/v1/package/gh/CYE04/Cecp@main/flat?t='+Date.now();
    const metaRes=await fetch(metaUrl,{cache:'no-store'});
    if(!metaRes.ok) throw new Error('歌曲目录载入失败 ('+metaRes.status+')');
    const meta=await metaRes.json();
    return (meta.files||[])
      .filter(f=>f&&/^\/songs\/[^/]+\.json$/i.test(f.name)&&!/\/test\.json$/i.test(f.name))
      .map(f=>{
        const name=f.name.split('/').pop();
        return {name,url:'https://cdn.jsdelivr.net/gh/CYE04/Cecp@main/songs/'+encodeURIComponent(name)};
      });
  }
  ...
  async function loadSongs(opts={}){
    try{
      ...
      const jsons=await getSongJsonEntries();
      const all=await Promise.all(jsons.map(f=>fetch(f.url,{cache:'no-store'}).then(r=>{
        if(!r.ok) throw new Error(String(r.status));
        return r.json();
      }).catch(err=>{
        console.warn('[musiclib] song skipped:',f.name,err);
        return null;
      })));
      songs=all.filter(Boolean).map(enrichSong);
      if(!songs.length) throw new Error('没有读取到任何歌曲');
      ...
      console.error('[musiclib] loadSongs failed',e);
      if(!opts.refresh) $('ml-loading').innerHTML='<div style="color:#ff3b30;font-size:14px">载入失败，请稍后刷新重试</div>';
      else showToast('刷新失败，请稍后重试');
    }
  }
  ...
  /* Keep score spacing byte-for-byte identical to musictool.
     Do not replace U+3164 or synthesize device-specific gap spans: Safari,
     iPadOS and iOS must receive the same text node as the editor preview. */
